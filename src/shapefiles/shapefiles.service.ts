...
export class ShapefilesService {
    ...
    try {
      ...
      this.getLayerName(nameshapefile)
        .then(async (res) => {
          ...
          exec(
            'shp2pgsql -a -s 4326 -I -W "latin1" /mnt/d/tmp/' +
              pathandfile +
  ...
  private uploadStyle(sldfile, layername) {
    ...
    // alternative
    exec(
      `curl -v -u ${process.env.SERVER_USER}:${process.env.SERVER_PASSWORD} -XPUT -H "Content-type: application/vnd.ogc.sld+xml" -d @${sldfile} ${url}`,
    );
  }
