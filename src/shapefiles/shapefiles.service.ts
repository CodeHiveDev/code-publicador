...
export class ShapefilesService {
    public shapeHandler(fileshape: any, pathandfile, folders3, nameshapefile, type) {
        // Copy S3 file to a temp storage
        exec('aws s3 cp s3://ho-backend-content-dev/'+folders3+' /mnt/d/tmp/'+folders3+ ' --recursive --exclude "*" --include "'+nameshapefile+'*"')
        // Convert shp to postgis
        try {
            // Create table
            exec('PGPASSWORD='+process.env.PGPASSWORD+' psql -h '+process.env.POSTGRES_HOST+' -d '+process.env.POSTGRES_DB+' -p '+process.env.POSTGRES_PORT+' -U '+process.env.POSTGRES_USER+' -c "CREATE TABLE IF NOT EXISTS public.shapefiles ( idshapefile serial NOT NULL, id_0 integer NOT NULL, layername varchar(100), expediente text, categoria text, fecha date, geom geometry, PRIMARY KEY (idshapefile))" '); // -f "createshapefile"
            // Altern Table add column
            exec('PGPASSWORD='+process.env.PGPASSWORD+' psql -h '+process.env.POSTGRES_HOST+' -d '+process.env.POSTGRES_DB+' -p '+process.env.POSTGRES_PORT+' -U '+process.env.POSTGRES_USER+' -c "ALTER TABLE public.shapefiles ADD COLUMN IF NOT EXISTS layername varchar(100) DEFAULT '+nameshapefile+' " ');
            // Shp Parser
            ...
            .then(async (res)  => {
                ...
                exec('shp2pgsql -a -s 4326 -I -W "latin1" /mnt/d/tmp/' + pathandfile + ' public.shapefiles | PGAPPNAME="'+nameshapefile+'" PGPASSWORD='+process.env.PGPASSWORD+' psql -h '+process.env.POSTGRES_HOST+' -d '+process.env.POSTGRES_DB+' -p '+process.env.POSTGRES_PORT+' -U '+process.env.POSTGRES_USER+' ');
                // Insert field layername if is null
                exec('PGPASSWORD='+process.env.PGPASSWORD+' psql -h '+process.env.POSTGRES_HOST+' -d '+process.env.POSTGRES_DB+' -p '+process.env.POSTGRES_PORT+' -U '+process.env.POSTGRES_USER+' -c "UPDATE public.shapefiles SET layername = '+"'"+nameshapefile+"'"+' WHERE layername IS NULL" '); //AND fecha = atributo fecha
                    ...
                    await this.createStyle(nameshapefile).then((res)=>{
                        // Load Style
                        let sldfile = `/mnt/d/tmp/${folders3}${nameshapefile}.sld`
    ...
    private async getLayerName(layername){
        ...
        let config = {
            method: 'get',
            url: `http://${process.env.SERVER_HOST}/geoserver/rest/workspaces/Mineria/datastores/postgis/featuretypes/${layername}.xml`,
            headers: { 
    ...
    private async publishLayer(nameshapefile, type){
        ...
        // Style layer
        // Post Config
        let url = `http://${process.env.SERVER_HOST}/geoserver/rest/workspaces/Mineria/datastores/postgis/featuretypes`
        let auth = `${process.env.SERVER_AUTH}`
    ...
    private createStyle(layername){
        return new Promise( async (resolve, reject) => {
            ...
            let url = `http://${process.env.SERVER_HOST}/geoserver/rest/styles`
            let auth = `${process.env.SERVER_AUTH}`
            const conn = await this.connection(dataStyle, 'post', url, auth, 'xml', func)
            if (conn  !== null) {resolve("res")}
        })
    };

    private uploadStyle(sldfile, layername){
        let func = "uploadStyle"
        let url = `http://${process.env.SERVER_HOST}/geoserver/rest/styles/${layername}` //?name=${layername}` ?raw=true
        let auth = `${process.env.SERVER_AUTH}`
        //this.connection(sldfile, 'put', url, auth, 'vnd.ogc.sld+xml', func)
        // alternative
        exec(`curl -v -u ${process.env.SERVER_USER}:${process.env.SERVER_PASSWORD} -XPUT -H "Content-type: application/vnd.ogc.sld+xml" -d @${sldfile} ${url}`)
    };
    ...
    private setLayerStyle(layername){
        ...
        let url = `http://${process.env.SERVER_HOST}/geoserver/rest/layers/Mineria:${layername}`
        let auth = `${process.env.SERVER_AUTH}`
